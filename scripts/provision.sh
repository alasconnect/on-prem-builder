...
start_api() {
  sudo -E hab svc load "${BLDR_ORIGIN}/builder-api/7554" --bind router:builder-router.default --channel "${BLDR_CHANNEL}" --force
}

start_api_proxy() {
  sudo -E hab svc load "${BLDR_ORIGIN}/builder-api-proxy/7519" --bind http:builder-api.default --channel "${BLDR_CHANNEL}" --force
}

start_datastore() {
  sudo -E hab svc load "${BLDR_ORIGIN}/builder-datastore/7311" --channel "${BLDR_CHANNEL}" --force
}

start_originsrv() {
  sudo -E hab svc load "${BLDR_ORIGIN}/builder-originsrv/7582" --bind router:builder-router.default --bind datastore:builder-datastore.default --channel "${BLDR_CHANNEL}" --force
}

start_router() {
  sudo -E hab svc load "${BLDR_ORIGIN}/builder-router/7519" --channel "${BLDR_CHANNEL}" --force
}

start_sessionsrv() {
  sudo -E hab svc load "${BLDR_ORIGIN}/builder-sessionsrv/7582" --bind router:builder-router.default --bind datastore:builder-datastore.default --channel "${BLDR_CHANNEL}" --force
}
...
start_minio() {
  ...
  sudo -E hab svc load "${BLDR_ORIGIN}/builder-minio/0.1.0" --channel "${BLDR_CHANNEL}" --force
